...
class EditRecurringScreen extends StatefulWidget {
  ...
  const EditRecurringScreen({Key? key, required this.recurringData})
      : super(key: key);
...
class _EditRecurringScreenState extends State<EditRecurringScreen> {
  ...
  void initState() {
    ...
    selectedReminderType =
        widget.recurringData.recurringType == 'Monthly' ? 0 : 1;
    if (widget.recurringData.recurringType == 'Monthly') {
      selectedDate = widget.recurringData.recurringDay - 1;
    } else {
      selectedWeekday = widget.recurringData.recurringDay;
    }
    ...
    selectedTime = DateTime(
      ...
      DateTime.now().month,
      DateTime.now().day,
      widget.recurringData.recurringHour,
      widget.recurringData.recurringMinute,
    );

    amountController.text = widget.recurringData.amount.toString();
  }
  ...
  addRecurring() async {
    ...
    res = await favoriteController.editRecurring({
